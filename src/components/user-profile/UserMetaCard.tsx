"use client";
import React, { useState, ChangeEvent, FormEvent } from "react";
import Image from "next/image";
import axios from 'axios'; // Importar axios
import { useAuth } from "@/context/AuthContext"; // Importar el hook de autenticación
import { useModal } from "../../hooks/useModal";
import { Modal } from "../ui/modal";
import Button from "../ui/button/Button";
import Input from "../form/input/InputField";
import Label from "../form/Label";
import { PencilIcon } from "@/icons"; // Importar icono de lápiz

// Interfaz para los datos del formulario del modal
interface MetaFormData {
  profile_picture_url: string;
  socialMedia: {
    facebook?: string;
    x?: string;
    linkedin?: string;
    instagram?: string;
  };
}

export default function UserMetaCard() {
  const { user, token, updateUser } = useAuth(); // Obtener usuario, token y función de actualización del contexto
  const { isOpen, openModal, closeModal } = useModal();
  const [formData, setFormData] = useState<MetaFormData>({
    profile_picture_url: user?.profile_picture_url || '',
    socialMedia: {
      facebook: user?.socialMedia?.facebook || '',
      x: user?.socialMedia?.x || '',
      linkedin: user?.socialMedia?.linkedin || '',
      instagram: user?.socialMedia?.instagram || '',
    },
  });
  const [isUploading, setIsUploading] = useState(false);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);
  const [saveError, setSaveError] = useState<string | null>(null);
  const [selectedFile, setSelectedFile] = useState<File | null>(null); // Para la previsualización

  // Manejador para cambios en inputs de redes sociales
  const handleSocialChange = (e: ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setFormData((prev) => ({
      ...prev,
      socialMedia: {
        ...prev.socialMedia,
        [name]: value,
      },
    }));
  };

  // Manejador para seleccionar archivo de imagen
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      setSelectedFile(e.target.files[0]);
      // Actualizamos la URL temporalmente para previsualización,
      // la URL final vendrá de Cloudinary
      setFormData((prev) => ({
          ...prev,
          profile_picture_url: URL.createObjectURL(e.target.files![0])
      }));
    } else {
        setSelectedFile(null);
        // Si no hay archivo, volvemos a la imagen original del usuario
        setFormData((prev) => ({
            ...prev,
            profile_picture_url: user?.profile_picture_url || ''
        }));
    }
  };

  // Función para subir a Cloudinary
  const uploadToCloudinary = async (file: File): Promise<string> => {
    setIsUploading(true);
    setUploadError(null);
    const cloudinaryFormData = new FormData();
    cloudinaryFormData.append('file', file);
    cloudinaryFormData.append('upload_preset', 'saveat_preset_cloudinary'); // <-- REEMPLAZA CON TU UPLOAD PRESET
    // Esto le dice a Cloudinary que guarde la imagen en esa carpeta
    cloudinaryFormData.append('folder', 'saveat_folder_profile');

    try {
      const response = await axios.post(
        `https://api.cloudinary.com/v1_1/isay83/image/upload`, // <-- REEMPLAZA CON TU CLOUD NAME
        cloudinaryFormData
      );
      setIsUploading(false);
      return response.data.secure_url; // URL segura de la imagen subida
    } catch (error) {
      console.error("Error uploading to Cloudinary:", error);
      setIsUploading(false);
      setUploadError('Error al subir la imagen. Inténtalo de nuevo.');
      throw error; // Propagamos el error
    }
  };

  // Manejador para guardar cambios (subir imagen si hay y llamar a la API)
  const handleSave = async (e: FormEvent) => {
    e.preventDefault();
    setIsSaving(true);
    setSaveError(null);
    let finalImageUrl = formData.profile_picture_url; // Usamos la URL actual (puede ser de previsualización o la original)

    try {
      // 1. Subir nueva imagen si se seleccionó una
      if (selectedFile) {
        finalImageUrl = await uploadToCloudinary(selectedFile);
      }

      // 2. Preparar datos para la API (solo URL y redes sociales)
      const dataToUpdate = {
        profile_picture_url: finalImageUrl,
        socialMedia: formData.socialMedia,
      };

      // 3. Llamar a la API del backend para actualizar
      const apiUrl = 'http://localhost:5000/api/v1/admins/profile';
      const response = await fetch(apiUrl, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`, // Enviar el token
        },
        body: JSON.stringify(dataToUpdate),
      });

      const updatedAdminData = await response.json();

      if (!response.ok) {
        throw new Error(updatedAdminData.message || 'Error al actualizar perfil');
      }

      // 4. Actualizar el contexto global (y localStorage)
      updateUser({
        profile_picture_url: finalImageUrl,
        socialMedia: formData.socialMedia,
      });

      // 5. Limpiar y cerrar modal
      setSelectedFile(null); // Limpiar archivo seleccionado
      closeModal();

    } catch (error) {
      console.error("Error saving profile:", error);
      if (error instanceof Error) {
        setSaveError(error.message);
      } else {
        setSaveError('Ocurrió un error inesperado al guardar.');
      }
    } finally {
      setIsSaving(false);
    }
  };

  // Función para abrir el modal y cargar los datos actuales del usuario
  const openEditModal = () => {
    setFormData({
      profile_picture_url: user?.profile_picture_url || '',
      socialMedia: {
        facebook: user?.socialMedia?.facebook || '',
        x: user?.socialMedia?.x || '',
        linkedin: user?.socialMedia?.linkedin || '',
        instagram: user?.socialMedia?.instagram || '',
      },
    });
    setSelectedFile(null); // Resetear archivo seleccionado
    setUploadError(null);
    setSaveError(null);
    openModal();
  }

  // Renderizado condicional mientras carga o si no hay usuario
  if (!user) {
    return <div>Cargando perfil...</div>; // O un spinner
  }

  const profileImageUrl = selectedFile ? formData.profile_picture_url : user.profile_picture_url || "/images/user/user.png";


  return (
    <>
      {/* --- Tarjeta Principal (Mostrar Datos) --- */}
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            {/* Imagen de Perfil */}
            <div className="relative w-20 h-20">
                <Image
                    width={80}
                    height={80}
                    src={profileImageUrl || "/images/user/user.png"} // Usa la URL del contexto o la default
                    alt="user"
                    className="object-cover w-full h-full overflow-hidden border border-gray-200 rounded-full dark:border-gray-800"
                    // Añadir un onError para fallback si la URL falla
                    onError={(e) => {
                        // Si falla la carga, muestra la imagen por defecto
                        const target = e.target as HTMLImageElement;
                        target.onerror = null; // Previene bucles infinitos
                        target.src = "/images/user/user.png";
                    }}
                />
                 {/* Botón flotante para editar imagen */}
                 <button
                    onClick={openEditModal}
                    className="absolute bottom-0 right-0 flex items-center justify-center w-6 h-6 bg-white border border-gray-300 rounded-full dark:bg-gray-800 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700"
                    aria-label="Edit profile picture"
                  >
                   <PencilIcon className="w-5 h-5 text-gray-600 dark:text-gray-300" />
                </button>
            </div>

            {/* Nombre, Rol, Ubicación */}
            <div className="order-3 text-center xl:order-2 xl:text-left">
              <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90">
                {user.first_name} {user.last_name}
              </h4>
              <div className="flex flex-col items-center gap-1 xl:flex-row xl:gap-3">
                 {/* Mostramos el rol (bio no existe en el modelo Admin) */}
                <p className="text-sm text-gray-500 dark:text-gray-400 capitalize">
                   {user.role}
                </p>
                {(user.city || user.country) && (
                    <>
                        <div className="hidden h-3.5 w-px bg-gray-300 dark:bg-gray-700 xl:block"></div>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                        {user.city}{user.city && user.country ? ', ' : ''}{user.country}
                        </p>
                    </>
                )}
              </div>
            </div>

             {/* Enlaces a Redes Sociales */}
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              {user.socialMedia?.facebook && (
                 <a href={user.socialMedia.facebook.startsWith('http') ? user.socialMedia.facebook : `https://${user.socialMedia.facebook}`} target="_blank" rel="noreferrer" className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"> {/* Clases CSS originales */}
                   {/* Icono Facebook SVG */}
                    <svg className="fill-current" width="20" height="20" viewBox="0 0 20 20"><path
                    d="M11.6666 11.2503H13.7499L14.5833 7.91699H11.6666V6.25033C11.6666 5.39251 11.6666 4.58366 13.3333 4.58366H14.5833V1.78374C14.3118 1.7477 13.2858 1.66699 12.2023 1.66699C9.94025 1.66699 8.33325 3.04771 8.33325 5.58342V7.91699H5.83325V11.2503H8.33325V18.3337H11.6666V11.2503Z"
                    fill=""
                  /></svg>
                 </a>
              )}
               {user.socialMedia?.x && (
                 <a href={user.socialMedia.x.startsWith('http') ? user.socialMedia.x : `https://${user.socialMedia.x}`} target="_blank" rel="noreferrer" className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"> {/* Clases CSS originales */}
                   {/* Icono X SVG */}
                    <svg className="fill-current" width="20" height="20" viewBox="0 0 20 20">
                      <path
                    d="M15.1708 1.875H17.9274L11.9049 8.75833L18.9899 18.125H13.4424L9.09742 12.4442L4.12578 18.125H1.36745L7.80912 10.7625L1.01245 1.875H6.70078L10.6283 7.0675L15.1708 1.875ZM14.2033 16.475H15.7308L5.87078 3.43833H4.23162L14.2033 16.475Z"
                    fill=""
                  />
                    </svg>
                 </a>
              )}
              {user.socialMedia?.linkedin && (
                 <a href={user.socialMedia.linkedin.startsWith('http') ? user.socialMedia.linkedin : `https://${user.socialMedia.linkedin}`} target="_blank" rel="noreferrer" className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"> {/* Clases CSS originales */}
                   {/* Icono LinkedIn SVG */}
                   <svg className="fill-current" width="20" height="20" viewBox="0 0 20 20">
                   <path
                    d="M5.78381 4.16645C5.78351 4.84504 5.37181 5.45569 4.74286 5.71045C4.11391 5.96521 3.39331 5.81321 2.92083 5.32613C2.44836 4.83904 2.31837 4.11413 2.59216 3.49323C2.86596 2.87233 3.48886 2.47942 4.16715 2.49978C5.06804 2.52682 5.78422 3.26515 5.78381 4.16645ZM5.83381 7.06645H2.50048V17.4998H5.83381V7.06645ZM11.1005 7.06645H7.78381V17.4998H11.0672V12.0248C11.0672 8.97475 15.0422 8.69142 15.0422 12.0248V17.4998H18.3338V10.8914C18.3338 5.74978 12.4505 5.94145 11.0672 8.46642L11.1005 7.06645Z"
                    fill=""
                  />
                   </svg>
                 </a>
              )}
               {user.socialMedia?.instagram && (
                 <a href={user.socialMedia.instagram.startsWith('http') ? user.socialMedia.instagram : `https://${user.socialMedia.instagram}`} target="_blank" rel="noreferrer" className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"> {/* Clases CSS originales */}
                    {/* Icono Instagram SVG */}
                    <svg className="fill-current" width="20" height="20" viewBox="0 0 20 20">
                      <path
                    d="M10.8567 1.66699C11.7946 1.66854 12.2698 1.67351 12.6805 1.68573L12.8422 1.69102C13.0291 1.69766 13.2134 1.70599 13.4357 1.71641C14.3224 1.75738 14.9273 1.89766 15.4586 2.10391C16.0078 2.31572 16.4717 2.60183 16.9349 3.06503C17.3974 3.52822 17.6836 3.99349 17.8961 4.54141C18.1016 5.07197 18.2419 5.67753 18.2836 6.56433C18.2935 6.78655 18.3015 6.97088 18.3081 7.15775L18.3133 7.31949C18.3255 7.73011 18.3311 8.20543 18.3328 9.1433L18.3335 9.76463C18.3336 9.84055 18.3336 9.91888 18.3336 9.99972L18.3335 10.2348L18.333 10.8562C18.3314 11.794 18.3265 12.2694 18.3142 12.68L18.3089 12.8417C18.3023 13.0286 18.294 13.213 18.2836 13.4351C18.2426 14.322 18.1016 14.9268 17.8961 15.458C17.6842 16.0074 17.3974 16.4713 16.9349 16.9345C16.4717 17.397 16.0057 17.6831 15.4586 17.8955C14.9273 18.1011 14.3224 18.2414 13.4357 18.2831C13.2134 18.293 13.0291 18.3011 12.8422 18.3076L12.6805 18.3128C12.2698 18.3251 11.7946 18.3306 10.8567 18.3324L10.2353 18.333C10.1594 18.333 10.0811 18.333 10.0002 18.333H9.76516L9.14375 18.3325C8.20591 18.331 7.7306 18.326 7.31997 18.3137L7.15824 18.3085C6.97136 18.3018 6.78703 18.2935 6.56481 18.2831C5.67801 18.2421 5.07384 18.1011 4.5419 17.8955C3.99328 17.6838 3.5287 17.397 3.06551 16.9345C2.60231 16.4713 2.3169 16.0053 2.1044 15.458C1.89815 14.9268 1.75856 14.322 1.7169 13.4351C1.707 13.213 1.69892 13.0286 1.69238 12.8417L1.68714 12.68C1.67495 12.2694 1.66939 11.794 1.66759 10.8562L1.66748 9.1433C1.66903 8.20543 1.67399 7.73011 1.68621 7.31949L1.69151 7.15775C1.69815 6.97088 1.70648 6.78655 1.7169 6.56433C1.75786 5.67683 1.89815 5.07266 2.1044 4.54141C2.3162 3.9928 2.60231 3.52822 3.06551 3.06503C3.5287 2.60183 3.99398 2.31641 4.5419 2.10391C5.07315 1.89766 5.67731 1.75808 6.56481 1.71641C6.78703 1.70652 6.97136 1.69844 7.15824 1.6919L7.31997 1.68666C7.7306 1.67446 8.20591 1.6689 9.14375 1.6671L10.8567 1.66699ZM10.0002 5.83308C7.69781 5.83308 5.83356 7.69935 5.83356 9.99972C5.83356 12.3021 7.69984 14.1664 10.0002 14.1664C12.3027 14.1664 14.1669 12.3001 14.1669 9.99972C14.1669 7.69732 12.3006 5.83308 10.0002 5.83308ZM10.0002 7.49974C11.381 7.49974 12.5002 8.61863 12.5002 9.99972C12.5002 11.3805 11.3813 12.4997 10.0002 12.4997C8.6195 12.4997 7.50023 11.3809 7.50023 9.99972C7.50023 8.61897 8.61908 7.49974 10.0002 7.49974ZM14.3752 4.58308C13.8008 4.58308 13.3336 5.04967 13.3336 5.62403C13.3336 6.19841 13.8002 6.66572 14.3752 6.66572C14.9496 6.66572 15.4169 6.19913 15.4169 5.62403C15.4169 5.04967 14.9488 4.58236 14.3752 4.58308Z"
                    fill=""
                  />
                    </svg>
                 </a>
              )}
            </div>
          </div>
          {/* Botón Editar General */}
          <button
            onClick={openEditModal} // Abrir el modal específico
            className="flex w-full items-center justify-center gap-2 rounded-full border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 lg:inline-flex lg:w-auto"
          >
             <PencilIcon className="w-5 h-5" /> {/* Usar el icono importado */}
            Edit
          </button>
        </div>
      </div>

      {/* --- Modal Específico para Editar Meta (Imagen y Redes Sociales) --- */}
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
        <form onSubmit={handleSave} className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Editar Imagen y Redes Sociales
            </h4>
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Actualiza tu foto de perfil y enlaces sociales.
            </p>
          </div>

          <div className="custom-scrollbar h-auto max-h-[60vh] overflow-y-auto px-2 pb-3">
            {/* Sección para Cambiar Foto */}
            <div className="mb-6">
                <Label htmlFor="profilePictureInput">Foto de Perfil</Label>
                 <div className="flex items-center gap-4 mt-1.5">
                    <Image
                        width={64}
                        height={64}
                        src={formData.profile_picture_url || "/images/user/user.png"} // Muestra previsualización o actual
                        alt="Preview"
                        className="object-cover w-16 h-16 rounded-full border border-gray-200 dark:border-gray-700"
                        onError={(e) => {
                            const target = e.target as HTMLImageElement;
                            target.onerror = null;
                            target.src = "/images/user/user.png";
                        }}
                    />
                    <input
                        id="profilePictureInput"
                        type="file"
                        accept="image/png, image/jpeg, image/jpg, image/webp"
                        onChange={handleFileChange}
                        className="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-brand-50 file:text-brand-700 hover:file:bg-brand-100 dark:file:bg-brand-500/10 dark:file:text-brand-400 dark:hover:file:bg-brand-500/20"
                    />
                 </div>
                 {isUploading && <p className="mt-2 text-sm text-blue-500">Subiendo imagen...</p>}
                 {uploadError && <p className="mt-2 text-sm text-error-500">{uploadError}</p>}
            </div>

            {/* Sección para Redes Sociales */}
            <div>
              <h5 className="mb-5 text-lg font-medium text-gray-800 dark:text-white/90 lg:mb-6">
                Enlaces Sociales
              </h5>
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                <div>
                  <Label htmlFor="facebookInput">Facebook</Label>
                  <Input
                    id="facebookInput"
                    name="facebook" // Debe coincidir con la key en formData.socialMedia
                    type="text"
                    placeholder="ej. https://facebook.com/usuario"
                    value={formData.socialMedia.facebook}
                    onChange={handleSocialChange}
                  />
                </div>
                <div>
                  <Label htmlFor="xInput">X (Twitter)</Label>
                  <Input
                    id="xInput"
                    name="x"
                    type="text"
                    placeholder="ej. https://x.com/usuario"
                    value={formData.socialMedia.x}
                    onChange={handleSocialChange}
                  />
                </div>
                <div>
                  <Label htmlFor="linkedinInput">LinkedIn</Label>
                  <Input
                    id="linkedinInput"
                    name="linkedin"
                    type="text"
                    placeholder="ej. https://linkedin.com/in/usuario"
                    value={formData.socialMedia.linkedin}
                    onChange={handleSocialChange}
                  />
                </div>
                <div>
                  <Label htmlFor="instagramInput">Instagram</Label>
                  <Input
                    id="instagramInput"
                    name="instagram"
                    type="text"
                    placeholder="ej. https://instagram.com/usuario"
                    value={formData.socialMedia.instagram}
                    onChange={handleSocialChange}
                  />
                </div>
              </div>
            </div>
          </div>

          {/* Botones y Mensaje de Error */}
          {saveError && <p className="mt-4 px-2 text-sm text-center text-error-500">{saveError}</p>}
          <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
            <Button size="sm" variant="outline" onClick={closeModal} type="button" disabled={isSaving || isUploading}>
              Cancelar
            </Button>
            <Button size="sm" type="submit" disabled={isSaving || isUploading}>
              {isSaving ? 'Guardando...' : 'Guardar Cambios'}
            </Button>
          </div>
        </form>
      </Modal>
    </>
  );
}
